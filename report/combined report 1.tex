...
Hypertension is when blood pressure is too high, leading to high stress and strains on the walls of the arteries and the heart.  There is an increased risk for further cardiovascular diseases such as strokes and heart attacks. Two measures of the blood pressure are combined.  One is the pressure when the heart muscles contract (systolic pressure). The other is when it relaxes (diastolic pressure).

An inital layout of the whole process is useful for dividing tasks into modular components.  Figure \ref{fig:TreatPath} is a readapted version of the primary care pathway for hypertension, based on NICE guidelines\cite{CG127}.  Primary care means that it is the main point of contact for patients with their healthcare provider. It is where the patient sees a general practioner of nurse first, before having any specialist treatment for complicated situations.  The pathway is shown to give a very broad overview of the whole process of dealing with hypertension.  In fact, such a workflow can be generalized to many other diseases. We aim to model most of these components, with finer details to be discussed later.
\begin{figure}[!p]
\begin{center}
...
The treatment here is summarised in a single block tenatively named "change treatment". Complex decisions need to be made. There are many factors which may affect how a physician decides to progress with the treatment.  For example, different conditions may indicate or contraindicate certain drugs. Or a patient may be allergic to some.  Financial and practical costs may also be taken into acccount.

Also, according to NICE guidelines, there could be 4 stages of drug treatments, before a patient is deemed to have resistant hypertension (need to refer to specialist).  Each of these 4 stages of drugs depend on the previous stage with some logical algorithm.  

At the end of this step, a patient is given a new treatment course, which includes a drug and its prescription.  
...
The key parameter is reduction in systolic and diastolic blood pressure. Adverse effects are also constanltly assessed to make sure the patient is reacting well.   If there is any noteworthy event, then the doctor is alerted. In usual care, these events is just a patient having an review appointment, or a patient contacting the doctor if symptoms of side effects are worsening.  In the case of remote monitoring, further alerts may be needed such as for patients not using the equipment according to protocol. Also, alerts would be more frequent leading to a tighter loop of control.

Two cycles of monitoring are in effect generally.  First is a short term cycle, which occurs once a treatment has changed.  The parameters of Blood pressure (and possible adverse effects) at this point is unstable, and a closer observation of its changes would allow quicker response.  Usual care would wait 4 weeks to see changes in BP, and even earlier to sometimes monitor adverse effects for certain drugs.  The treatment keeps changing according to the rules mentioned above, until treatment is sufficient.

This is when the patient enters the second longer cycle of monitoring.  Now the parameters are in a steady stable state, as long as the patient keeps following their treatment regime.  Normally patients are reviewed annually at this stage in order to see if hypertension has relapsed.  The relapse has a higher chance of occuring if the review is at a longer time frame.  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\renewcommand{\arraystretch}{1.2}
    ...
    \\ \hline
	 ...
	 \\ \hline
    \end{tabular}
\caption{Key Drivers for Telemonitoring}
...
\end{tabular}
\caption{Telemonitoring Studies}
\label{table:TMtechs}
\end{table}

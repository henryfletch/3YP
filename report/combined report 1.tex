...
\usepackage{courier}
\usepackage{float}
%\floatstyle{boxed} << This used to put a box around Emma's code
\newfloat{code}{ht}{lop}
...
\textsc{\Large Intelligent software for medical diagnosis and treatment}\\[0.5cm]
% Title
\HRule \\[0.4cm]
...
%% ENG DECLARATION OF AUTHORSHIP FORMS - MANDATORY! ONE FOR EACH PERSON!
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Executive Summary - \textit{Michael Fedosiuk}}

There has been a steep rise in the development of software and applications aimed at both patients and doctors alike to aid treatment and diagnosis. Software has been developed to help free up time pressured GP's by condensing the vast rapidly increasing quantities of medical research literature and through the telemonitoring of patients.  

The project was linked with OpenClinical, "An international organisation that has been created to promote awareness and use of decision support, clinical workflow and other advanced knowledge management technologies for patient care and clinical research." \cite{openclin}  To this end the goal was set to build an intelligent software application which would aid either the treatment or diagnosis of medical conditions.

From this "Cardiac Track" was created, an integrated solution for prolonged up to date hypertension treatment. It consists of two applications that work in tandem, one for the doctor and one for the patient. The later requires the patient to measure their own blood pressure (BP) at home each day and then enter this reading into the application. If the patients BP reading breaches the level which is deemed safe, set by the doctor's application, both patient and doctor are alerted. (The patient is prompted to make an appointment, with their doctor's contact details.) On visiting the surgery the second application can be used by the doctor to track how the patient's BP has varied over time, check the uncertainty of the data that the patient has inputted and finally prescribe a new antihypertensive medication. The same process is then iterated, to ensure the patients BP remains controlled below the safe target level.

The aim is to provide up to date information and treatment for a patient's hypertensive condition, and hence reduce the prevalence of uncontrolled hypertension and so the associated conditions of stroke and ischemic heart disease. Through the course of the project a fully functioning prototype application was built in the languages of HTML, CSS, PHP and OpenClinical's specialist Tallis software. This report outlines the medical background, the clinical need, the technical implementation of the application and finally its economic viability.  
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Hypertension Care Pathway - \textit{Jeffrey Yu}}

\subsection{What is Hypertension}
Hypertension is when blood pressure is too high, leading to high stress and strains on the walls of the arteries and the heart.  There is an increased risk for further cardiovascular diseases such as strokes and heart attacks. Two measures of the blood pressure are combined.  One is the pressure when the heart muscles contract (systolic pressure). The other is when it relaxes (diastolic pressure).  As a general case, hypertension is diagnosed when the reading is above 140/90 mmHg  (systolic/diastolic) at the clinic. 

With hypertension being a chronic disease, an inital layout of the whole process is useful for dividing tasks into modular components.  Figure \ref{fig:TreatPath} is a readapted version of the primary care pathway for hypertension, based on NICE guidelines\cite{CG127}.  Primary care means that it is the main point of contact for patients with their healthcare provider. It is where the patient sees a general practioner of nurse first, before having any specialist treatment for complicated situations.  The pathway is shown to give a very broad overview of the whole process of dealing with hypertension.  In fact, such a workflow can be generalized to many other diseases. Below, a general description is given, with finer details to be discussed later.
\begin{figure}[!p]
\begin{center}
\includegraphics[scale=0.30]{TreatPath}
\caption{Overview of treatment pathway}
\label{fig:TreatPath}
\end{center}
\end{figure}
\subsection{Diagnosis}
A new patient would come for their health checkup. If the blood pressure is too high, other assessments are performed.  This blood pressure threshold is based on some of the patients background medical history.   Also, the guideline is only applicable to certain conditions in addition to hypertension.  For example, a patient who has hypertension as a result of other illnesses such as diabetes, should follow a treatment plan for that illness instead. Or if the patient has a condition that may interact with the treatment of hypertension, a specialist may be needed in secondary care.  At the end of the diagnosis, the patient would go back to a long term review if blood pressure is under control. Or in the case of our app where the patient is diagnosed, treatment of hypertension is started.
\subsection{Treatment}
The treatment here is summarised in a single block tenatively named "change treatment". Complex decisions need to be made on how to choose the drugs. There are many factors which may affect how a physician decides to progress with the treatment.  For example, different conditions may indicate or contraindicate certain drugs. Or a patient may be allergic to some.  Financial and practical costs may also be taken into acccount.

At the end of this step, a patient is given a new treatment course, which includes a drug and its prescription. The new treatment could simply be an increase in dosage until hypertension is controlled (a process known as titration). Or if this drug has reached a maximum tolerable dose, a new drug is added. Either way, it is simply labled as a "treatment change" for now.  Also, according to NICE guidelines, there could be 4 stages of drug treatments, before a patient is deemed to have resistant hypertension (need to refer to specialist).  Each of these 4 stages of drugs depend on the previous stage with some logical algorithm.  
...
The key parameter is reduction in systolic and diastolic blood pressure. Adverse effects are also constantly assessed to make sure the patient is reacting well.   If there is any noteworthy event, then the doctor is alerted. In usual care, these events is just a patient having an review appointment, or a patient contacting the doctor if symptoms of side effects are worsening.  In the case of remote monitoring, further alerts may be needed such as for patients not using the equipment according to protocol. Also, alerts would be more frequent leading to a tighter loop of control.

Two cycles of monitoring are in effect generally.  First is a short term cycle, which occurs once a treatment has changed (new drug, or dose increased).  The parameters of Blood pressure (and possible adverse effects) at this point is unstable, and a closer observation of its changes would allow quicker response.  Usual care would wait 4 weeks to see changes in BP.  For certain drugs, adverse effects are monitored even earlier at about 2 weeks to perform blood tests to ensure kidney function is normal.  Until treatment is sufficient, the treatment keeps adjusting according to the change of the parameters of blood pressure and adverse effects.

Once blood pressure is under control, patient enters the second longer cycle of monitoring.  Now the parameters are in a steady stable state, as long as the patient keeps following their treatment regime.  Normally patients are reviewed annually at this stage in order to see if hypertension has relapsed.  The relapse has a higher chance of occuring if the review is at a longer time frame.  

For both these cycles, the blood pressure is only measured at the end of the wait, and complications could happen before that. But by having data available more frequently, say daily, the treatment could potentially be improved by shortening this loop of control.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\renewcommand{\arraystretch}{1.2}
    ...
    \hline
    \textbf{Drivers} & \textbf{Explanation} 
    \\ \hline
    ...
    \\ \hline
     ...
     \\ \hline
     Coaching & Motivational interviewing and other techniques to encourage patient behavioural change and self-care
     \\ \hline
	 ...
	 \\ \hline
    \end{tabular}
\caption{Key Drivers for Telemonitoring\cite{TM2}.  }
\label{table:TMproscons}
\end{table}

In general patients would be able to send more data for health practitioners to analyse and react accordingly. There would be a shorter time frame for action, which hopefully would result in more reliable treatment.  One of the largest (3230 patients) trials recently was the "Whoe System Demonstrator" trial performed by the department of health. Chronic diseases such as diabetes, chronic obstructive pulmonary disease, and heart failure was monitored remotely. It resulted in lower lower emergency admission, and lower mortality rates.  
\\ \indent
...
\subsection{Hypertension Telemonitoring studies}
...
\\ \indent
Different technologies and designs are analysed to find any patterns for positive results. Table \ref{table:TMtechs} (page \pageref{table:TMtechs}) is a group of the different technologies summarised by AbuDagga et al. In essence, the overarching procedure involves the patient taking readings at home with a blood pressure monitor, transferring data through a telephone line or internet to a secure server, an alerting system for worthy events and a researcher/nurse/physician responding to such events.

The BP monitoring devices could be stand-alone, which would require input from the patient. Or it could wired/wirelessly connect to other devices. Some were able to send via bluetooth.  The monitoring devices with less user input had a better adherence rates, but coming at a higher price.  
...
\end{tabular}
\caption{Telemonitoring Studies by  summarised by AbuDagga et al\cite{impact Lit}}
\label{table:TMtechs}
\end{table}
 \clearpage 
...
Smartphone compatible blood pressure monitors are also available \cite{app2,app3}. They have the benefit of being able to send blood pressure (and heart rate) data directly to a smartphone.  But along with the high price tag, another problem is that the app and measuring device usually rely heavily on each other. There is no manual input and there is no display on the device. Nevertheless, it seems more convenient to the patient, and could possibly improve compliance of monitoring.  


On the market, there are also patient side apps available.  They have a method to record blood pressure readings easily. But it is not connected to the healthcare provider.

It would be useful to have a system where a doctor's app could both monitor and recommend decisions. While a patient app could connect and feed data remotely.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\item The application must provide a submission form, allowing patients to enter their systolic and diastolic blood pressure values. This data must be uploaded to the database, along with the time and date of upload. Note: this may be extended to include other metrics which may be of value (e.g. heart rate, weight etc.).
...
\item The application must provide the contact details for the patient's doctor.
...
\item The application must be hosted on a suitable server.
\item The application must have a secure login system, which identifies the doctor and grants them access to view only their own patients' profiles.
\item The application must be able to read-from and write-to the shared database.
...
\item The application must allow the doctor to review the patient
    ...
    \item The application must recommend a secondary drug treatment or allow the doctor to update the dosage of any previously prescribed drugs if the patient's target blood pressure has not been met.
...
\\ \indent
The second part is a doctor app, currently optimised for desktops and tablets, which allows the doctor to check up on the details of any of their patients. It alerts them to which have breached their targets, and provides perscription suggestions on how to best medicate their hypertension on an individual basis. 
...
\item
The application is most likely to be used by patients in home or at work when they take BP measurement and by the doctor when within their surgery. Therefore in most use cases Internet connectivity can be assumed and the app's main function of updating the database requires it.
\end{itemize}
...
\begin{figure}[h!] 
\minipage{\textwidth}
\includegraphics[width=0.9\linewidth]{csslink.png}
...
\centering
\includegraphics[width=0.75\linewidth]{exampleclass.png}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.85\linewidth]{patientUI.png}
...
\subsection{Function - \textit{Emma Saragossi}}
...
\subsubsection{Optimizing layout for multiple browser dimensions - \textit{Emma Saragossi}}
\label{sec:CSS}
...
\subsubsection{Text formatting - \textit{Emma Saragossi}}
...
\subsubsection{Logo image - \textit{Emma Saragossi}}
...
\subsection{Login page - \textit{Emma Saragossi}}
...
\subsubsection{Internal pages - \textit{Emma Saragossi}}
...
\subsection{Creating a patient profile - \textit{Emma Saragossi}}
...
\subsubsection{Initial design - \textit{Emma Saragossi}}
...
\begin{table}[ht]
\begin{center}
\begin{tabular}{|p{1.8cm}|p{1.5cm}| l | l |p{4.3cm}|}
\hline
...
The network of specific tasks is shown in Figure~\ref{fig:tallis_task_top}.
...
\subsubsection{Separate patient creation form - \textit{Emma Saragossi}}
\label{tallis:separatenewuser}
...
\subsection{News feed - \textit{Emma Saragossi}}
...
The PHP function \textbf{patientsidebar} which generates the news feed is shown in Extract~\ref{code:PHPsidebar}. The function first connects to the database. Once the MySQL connection parameters have been configured and a new MySQL instance has been established, the alerted patients are identified by their patient ID (PHP variable: \textbf{\$id}). Alerted patients are all those in the table \textbf{patientInfo} whose bloodPpressure is not controlled and whose doctor is identified by the function parameter \textbf{\$doctorID} (the doctor currently logged in to the site).
...
\subsection{Patient search bar - \textit{Emma Saragossi}}

Embedded in the header on all pages is the patient search bar, which allows the doctor to search by patient ID for immediate access to a patient's blood pressure and treatment history. See Section \ref{drapp:MF} and Figure~\ref{searchbarhtml} for further details. 
...
\subsection{Extension ideas for the Doctor Application - \textit{Emma Saragossi}}
...
\subsection{Patient checking and review - \textit{Michael Fedosiuk}} \label{drapp:MF}
\begin{figure}[h!] 
...
\centering
\includegraphics[scale=0.69]{selectfunc.png}
...
\begin{figure}[h!]
\centering
\includegraphics[scale=0.6]{searchbarhtml.png}
...
\subsubsection{Graph - \textit{Michael Fedosiuk}}
...
\centering
\includegraphics[scale=0.7]{tutorialCSS.png}
\caption{Tutorial section 1 CSS \label{tutcss}} 
\includegraphics[scale=0.7]{tutorial1jQuery.png}
...
\end{equation}
\begin{equation} \label{mike:equation18}
...
\begin{wrapfigure}{l}{.60\textwidth}
\centering
\includegraphics[scale=0.50]{mainSQL.png}
...
\\ \indent
A further step up the page into the next nest segment is where the calculation of both linear regression coefficients, $m_0$ and $m_1$, takes place. This is the implementation of equations 10 and 15 seen within previous linear regression section 7.3.1. The values of the variances $\Sigma x^2 -N\overline{y}\overline{y}, \Sigma y^2 - N\overline{y}\overline{y}, \Sigma xy - N\overline{x}\overline{y}$ are also calculated within and named with the prefixes ss\_xx, ss\_yy and ss\_xy. These are then called in the final nest segment to find the correlation coefficient and so complete the implementation of equation \ref{mike:equation18}, page \pageref{mike:equation18}.
\\ \indent
...
\centering
\includegraphics[scale=0.4]{unbar.png}
...
There are a couple of ways the current system may be improved upon if more data were present to analyse. Firstly it may be possible to devise whether there are long term trends in patients reactions to hypertension drugs, i.e. does their effectiveness drop over time. It may also be possible to collaborate with work such as the BioSign monitoring system being developed at OBS Medical Ltd. \cite{OBS} by Prof Lionel Tarassenko. This system tracks the vital signs of a patient in hospital and alerts nurses when the combination of measurements breach calculated bounds. With a large amount of data which is hoped to be amassed in the Cardiac Track app it may be possible to implement the uncertainty metrics in a similar way to BioSign by setting error bounds using a histogram of large data sets rather than a linear regression. 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Tallis application - \textit{Jeffrey Yu}}
Overall, the Tallis web application component is used to make the medical decisions during the treatment cycle.  First, an introductory description of the Tallis software and it's limitations are provided.  Then the implementation of the subcomponents are illustrated. 

\subsection{Scope of Tallis}

Tallis is  user-friendly, high level and less technical. It excels at linking and representing the sequential workflow that a lot of medicine is. At the same time, it preserves some flexibility in the components, which is needed in such a highly complex medical knowledgebase.

Tallis originally was presented as the backbone software for developing the app. Ideally, it would be convenient to have all the required components in one package.  This would reduce the interface with external components, thus minimising any potential errors and simplifying the software for further update.The problem however, stems from the fact that the project aims to encompass the whole care pathway for hypertension.  
...
\end{center}
\end{figure}
There are two main trees of enquiries, separated into two subplans. The first subplan is the "Treatment Status", where the treatment specific details (drug name, prescription etc.) of the patient are read from the records. Table \ref{table:treatmentstatus} shows all the data items read. It is constructed in such a sequence so that the correct amount of information is read every time. Number of drugs needs to be read before reading the details of drug X. For example, if the patient is only taking 2 drugs, the reading sequence ends at "drug2" enquiry task and does not read "drug3" through the precondition (Number of drugs  >= 3). Otherwise, a null will be read from the database, which could possibly affect future tasks.  Alternatively, the task could be designed to read the 4 drugs in parallel instead of series, since any unnecessary task would be cancelled by preconditions anyway. Series is preferred because it ensures drug1 is read before drug2, adding an extra line of safety.  

The second subplan is the "Patient Details", which is the data for all other related conditions of the patient that may affect future decision making processes.  It is directly taken from the previously made New Patient component, which now is transposed to outside of Tallis as shown in section \ref{tallis:separatenewuser}.
...
This plan is activated whenever it is necessary to decide on a treatment change.  Figure \ref{fig:tallisTreatmentChange} shows the structure of this task.   Dosage Alert at the start presents the existing treatment and other patient details that has been read from the database previously.  By reminding the doctor of the status of the patient, it can serve as a safety mechanism to double check. "Update Status" writes the results to the database. The meat of the component is the two main plans: "New Drug" and "Dosage Change", both of which decide on a drug and its prescription using logic adapted from guidelines.
...
Following the NICE guidelines, there are 4 stages of drug treatment. A patient is incrementally given drugs until the blood pressure is controlled. Once it is controlled, the patient keeps taking the medication. To illustrate, for example, first a single drug is given. Assuming there are no adverse effects (explored in section \ref{jeff:checkup}), it should take 4 weeks for the drug to take effect noticeably. The BP is still not controlled, so the dose is increased slightly. The process is repeated until control is achieved, or a maximum safe dosage is reached.  If it still not controlled, a further stage of drug is prescribed (following a given rule). Again it repeats until all 4 drugs are at a maximum, which is a suitable time to label the patient as having "resistant hypertension". Referral to a specialist is recommended at this point.  

The cycle is over a long time, and a single run through this task only realizes a single step in changing drug treatment.  Although choosing a new drug, and changing the prescription seem like 2 separate steps, they can still be grouped together into the abstraction of  "treatment change" to give a neater loop and clearer overall structure.
...
With the initial treatment in hand, the further stages are actually simpler to implement.  The contraindications for any drug will be invariant over all 4 stages.  Adding an extra drug to the treatment would not make previous ones safe from a comorbidity, therefore much of the previous logical arguments could be kept.
...
The arguments that give a suggestive for/against weighting would need to be re-examined for further stages.  For example, age group is no longer an applicable argument past the initial treatment for most drugs.  Then new arguments need to be created to model the new rules at each stage. Figure \ref{fig:tallisNewDrug} is a summary taken from NICE guidelines.  There are slightly more complexities, especially when inspecting individual drugs \cite{CKS}. These further drug evidence is provided by NICE CKS (clinical knowledge summaries).  But overall, the rules are between drug classes and not between specific drugs.  Figure \ref{fig:tallisDrugArg2} shows the arguments that have been changed or added to the drug Ramipril.  For each of the drug stages, the logical AND combines the new rule with the number of drugs . As can be seen, only 6 new arguments had to be added, while keeping the 9 other comorbidities the same.  

\begin{figure}[ht]
\begin{center}
\includegraphics[scale=0.75]{tallisDrugArg2}
\caption{New logical arguments for drug decisions, based on further stages of treatments}
...
Preventative measures like this is implemented by including a variable which is a conditional to the cycling of this task. To illustrate the algorithm
\begin{enumerate}
...
\subsection{Checkup} \label{jeff:checkup}
...
The tabbed UI shows the basic tasks in a clearer interface with tabs at the top, as opposed to a menu on the left hand side of the legacy template.
...
During the initial stages of the project, the patients iPhone application was built to use AES encryption using the MySQL "AES\_ENCRYPT" command. We chose to use the patient's login password as the encryption key since the plain text password was not stored on the database server, and so if the database was compromised, recovering the encrypted data would be essentially impossible. Extract \ref{mysqlencrypt} shows part of the submission code and SQL statement (shown in red) that was used to encrypt the submitted data before it was saved onto the database.
...
\subsubsection{Implementation difficulties and subsequent removal from test system} \label{henry:encryption}

Unfortunately because of the nature of this project, encrypting the data would prove to be very difficult, since one piece of data needs to be accessible to multiple people (i.e. doctor and patient). Additionally, the need for the database access to be compatible with both the application website and the Tallis medical decision engine, meant that any data encryption had to occur using MySQL itself. The default version of AES encryption used by MySQL is 128-bits, rather than the 256-bits required by the NHS. MySQL can however be compiled to use AES-256 if required.
\\ \indent
...
Extract \ref{sql-injection-1} shows a very simple system where a user submits their username, and their password is then retrieved from the database. However, this code is very susceptible to an SQL injection attack. Suppose the user entered into the website form the following:
...
This section is to show different ways in which the project could be taken forward commercially. Firstly by stating the benefits it has to potential customers, the NHS is focused on however the same arguments apply to a variety of international markets. Secondly the analysis of the initial cost and future savings predicted from implementing the application. Then a potential rough pricing model and finally what the next steps would be.
...
\\
It is stated  "a reduction of 5 mmHg in systolic pressure is associated with a reduction in ischaemic heart disease incidence of 11\%" \cite{isch}. This means that even if a reduction to below 140 mmHg is not attainable for some patients, continued treatment to create a 5 mmHg drop would still have markable effects. The NICE report made a conservative estimate of a 4\% reduction in ischaemic heart disease through better treatment. Agreeing with this assumption it followed that with 300000 hospitalisations each year a 4\% reduction is 12000 admittances each given the average direct to the NHS of \pounds 2582 (in 2006 \pounds 2107). This gives a predicted yearly saving of \pounds 30.98 Million (in 2006 \pounds 25.2 Million). This figure is conservative as it does not incorporated any primary, outpatient or coronary care.
\\
\minipage{\textwidth}
\centering
...
\\ \indent 
No comparable app has the Cardiac Track App unique selling point is the integrated doctor's system. It is the ability to alert the patient and doctor to when a patients hypertension is putting them at risk, that adds value for the NHS in the prevention of Strokes and Ischaemic Heart Disease. Given the unique competency it seems reasonable to set a high user price for an app that is completed and fully maintained (i.e. the first pricing model suggestion) at \pounds 3 per user. Cutting 30\% for placement as a native app on iTunes. It must then be realised that in selling directly to the NHS there is only a single major customer, and they will be buying the app user licences in bulk hence a large discount rate from a user price would have to be agreed upon. If this is set at a 70\% discount from the most expensive application it would give a price per unit of 63p. If it were assumed that the application would be rolled out to 10\% of patients in the first year revenue of \pounds 2.67 Million would be generated. 
...
\end{wrapfigure}
The salaries for developers to finish and maintain the app would have to be deducted for 5 full time web developers; on the average salary of \pounds 40000 \cite{salary} this would cost \pounds 200000 in total. We require 4 servers to have one dedicated Apache and MySQL and redundancies for both. A suitable server for the predicted number of initial users would be the  would be the PowerEdge R420 costing \pounds4656 for all four \cite{dell}. To use MySQL enterprise edition a licence must be purchased for \pounds3000 \cite{MySQL}. Including a generous contingency this predicts a first year pre tax profit of \pounds2.66 Million if the NHS were to push for implementation of the project at the projected price, fig.\ref{costing}.

\section{Conclusion - \textit{Emma Saragossi}}
When challenged with the task of developing a software tool to aid a clinical diagnostic or treatment pathway, we chose to design a remote care application for treating hypertension. In the early development stages, our focus was telemonitoring, and we began developing an application which would allow patients to regularly upload their blood pressure values from a non-clinical setting. These could then be monitored remotely by a doctor to aid treatment selection. A Tallis tool to implement the treatment protocol for primary hypertension in adults was developed separately. The design naturally evolved to incorporate the two functions to provide a holistic treatment tool for hypertension. 

We believe we have designed an application with real value; there is a clinical need and a place for our application in the market. In its prototype stage, our application has several distinctive competencies. The doctor is provided with a wealth of data on a patient's blood pressure history, meaning treatment decisions no longer need to be based on a single blood pressure reading made at the time of the consultation. In addition, the alert system for patients with an unexpected rise in blood pressure ensures that patients receive medical attention when it is required and don't go unseen for long periods of time if there is a cause for concern.

\subsection{Project Limitations - \textit{Henry Fletcher}}
The project was limited due to both time and resource constraints, as well as limits on technical possibility. A significant part of the project, the mandated encryption of patient data (Section \ref{henry:encryption}), was not possible for us to implement in the prototype environment. With the requisite resources and knowledge, we feel that creating an encryption system would be feasible if the application were to be deployed for real use. 
\\ \indent
Another area which is mostly unexplored during this prototype stage is the scalability of the system, and how it could be used by many thousands of users simultaneously. Whilst we have made attempts at identifying potential scalability issues, only a real world trial could actually identify these. We were, for example, unable to test the application with thousands of users at this early stage.
\\ \indent
Finally, many more features could be added in the future, such as geolocation information to check validity of entered data, a more polished user interface, better handling of large numbers of patient's on the doctor website, and the expansion of the patient application to include compatibility with other devices. We feel these sorts of extension ideas could be explored and easily added if more time and resources were made available, and to ultimately bring this software out of a prototype and into a trial environment. 
...
\bibitem{openclin}
Thomson, R. (2014). OpenClinical: knowledge management technologies for healthcare. [online] Openclinical.org. Available at: url{http://www.openclinical.org} [Accessed 2 May. 2014].
\bibitem{webmd1}
...
\bibitem{linregdia}
Marin.edu, (2014). Chapter2--Correlation and Regression. [online] Available at: url{http://www.marin.edu/~npsomas/Lectures/Ch\_2/Sections\_01to04.htm} [Accessed 24 Apr. 2014].
\bibitem{Kryzig}
...
\bibitem{impact Lit}
AbuDagga A, Resnick H, Alwan M.2010. Impact of blood pressure telemonitoring on hypertension outcomes: a literature review. Telemedicine and e-Health. 16(7):830--838. 
\bibitem{tasminh2experience}
Jones M, Greenfield S, Bray E, Baral- Grant S, Hobbs F, Holder R et al. 2012.Patients' experiences of self-monitoring blood pressure and self-titration of medication: the TASMINH2 trial qualitative study. British Journal of General Practice. 62(595):135--142. 
\bibitem{TM1}
McKinstry B, Hanley J, Wild S, Pagliari C, Paterson M, Lewis S et al. 2013.Telemonitoring based service redesign for the management of uncontrolled hypertension: multicentre randomised controlled trial. BMJ: British Medical Journal. 346. 
\bibitem{TM2}
Coye M, Haselkorn A, DeMello S. 2009.Remote patient management: technology-enabled innovation and evolving business models for chronic disease care. Health Affairs. 28(1):126--135. 
\bibitem{TM3}
Steventon A, Bardsley M, Billings J, Dixon J, Doll H, Hirani S et al. 2012. Effect of telehealth on use of secondary care and mortality: findings from the Whole System Demonstrator cluster randomised trial. BMJ: British Medical Journal. 344. 
\bibitem{TM4}
S, ers C, Rogers A, Bowen R, Bower P, Hirani S et al. 2012. Exploring barriers to participation and adoption of telehealth and telecare within the Whole System Demonstrator trial: a qualitative study. BMC health services research. 12(1):220. 
\bibitem{TM5}
...
\bibitem{TM6}
Par\'e G, Jaana M, Sicotte C. 2007. Systematic review of home telemonitoring for chronic diseases: the evidence base. Journal of the American Medical Informatics Association. 14(3):269--277. 
\bibitem{Im1}
Artinian N, Washington O, Templin T. 2001. Effects of home telemonitoring and community-based monitoring on blood pressure control in urban African Americans: a pilot study. Heart \& Lung: The Journal of Acute and Critical Care. 30(3):191--199. 
\bibitem{Im2}
 Artinian N, Flack J, Nordstrom C, Hockman E, Washington O, Jen K et al. 2007. Effects of nurse-managed telemonitoring on blood pressure at 12-month follow-up among urban African Americans. Nursing research. 56(5):312--322. 
\bibitem{Im3}
Bobrie G, Postel-Vinay N, Delonca J, Corvol P. 2007. Self-Measurement and Self-Titration in Hypertension* A Pilot Telemedicine Study. American journal of hypertension. 20(12):1314--1320. 
\bibitem{Im4}
Bondmass M, Bolger N, Castro G, Avitall B. 2000. The effect of home monitoring and telemanagement on blood pressure control among African Americans. Telemedicine Journal. 6(1):15--23. 
\bibitem{Im5}
Bosworth H, Olsen M, Grubber J.2009.  Two self-management interventions to improve hypertension control: A randomized trial. Ann Intern Med. 151:687-695. 
\bibitem{Im6}
Carrasco M, Salvador C, Sagredo P, M\'arquez-Montes J, de Mingo M, Fragua J et al. 2008.  Impact of Patient--General Practitioner Short-Messages-Based Interaction on the Control of Hypertension in a Follow-up Service for Low-to-Medium Risk Hypertensive Patients: A Randomized Controlled Trial. Information Technology in Biomedicine, IEEE Transactions on. 12(6):780--791. 
\bibitem{Im7}
...
\bibitem{Im8}
Fursse J, Clarke M, Jones R, Khemka S, Findlay G.  2008. Early experience in using telemonitoring for the management of chronic disease in primary care. Journal of telemedicine and telecare.14(3):122--124. 
\bibitem{Im9}
 Green B, Cook A, Ralston J, Fishman P, Catz S, Carlson J et al. 2008. Effectiveness of home blood pressure monitoring, Web communication, and pharmacist care on hypertension control: a randomized controlled trial. Jama. 299(24):2857--2867. 
\bibitem{Im10}
Logan A, McIsaac W, Tisler A, Irvine M, Saunders A, Dunai A et al. 2007. Mobile Phone--Based Remote Patient Monitoring System for Management of Hypertension in Diabetic Patients*. American journal of hypertension. 20(9):942--948. 
\bibitem{Im11}
Madsen L, Kirkegaard P, Pedersen E. 2008. Blood pressure control during telemonitoring of home blood pressure. A randomized controlled trial during 6 months. Blood pressure. 17(2):78--86. 
\bibitem{Im12}
Parati G, Omboni S, Albini F, Piantoni L, Giuliano A, Revera M et al. 2009. Home blood pressure telemonitoring improves hypertension control in general practice. The TeleBPCare study. Journal of hypertension. 27(1):198--203. 
\bibitem{Im13}
Park M, Kim H, Kim K. 2009. Cellular phone and Internet-based individual intervention on blood pressure and obesity in obese patients with hypertension. International journal of medical informatics. 78(10):704--710. 
\bibitem{Im14}
Rogers M, Small D, Buchan D. 2001. Home monitoring service improves mean arterial pressure in patients with essential hypertension. A randomized, controlled trial. Ann Intern Med. 134:102--1032. 
\bibitem{Im15}
 Rinfret S, Lussier M, Peirce A, Duhamel F, Cossette S, Lalonde L et al. 2009. The impact of a multidisciplinary information technology--supported program on blood pressure control in primary care. Circulation: Cardiovascular Quality and Outcomes. 2(3):170--177. 
\bibitem{Lit1}
Dorr D, Bonner L, Cohen A, Shoai R, Perrin R, Chaney E et al. 2007. Informatics systems to promote improved care for chronic illness: a literature review. Journal of the American Medical Informatics Association. 14(2):156--163. 
\bibitem{app1}
